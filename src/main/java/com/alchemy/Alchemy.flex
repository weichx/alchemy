...
NUMBER_FLOAT=([0-9]+(f|m)) | ([0-9]+\.[0-9]+(f|m))
NUMBER_INTEGER=[0-9][Ll]?+
NUMBER_UNSIGNED=[0-9]+(UL|ul|u)?
...
<YYINITIAL, LONG_TEMPLATE_ENTRY> "final"                { return FINAL; } // todo -- rename to 'sealed'
<YYINITIAL, LONG_TEMPLATE_ENTRY> "finally"              { return FINALLY; } // todo -- we maybe don't have try/catch, just traps until wasm supports it
...
<YYINITIAL, LONG_TEMPLATE_ENTRY> "super"                { return SUPER; } // todo -- base
...
<YYINITIAL, LONG_TEMPLATE_ENTRY> {NUMBER_FLOAT}    { return NUMBER_FLOAT; }
<YYINITIAL, LONG_TEMPLATE_ENTRY> {DOUBLE_LITERAL}  { return DOUBLE_LITERAL; }
<YYINITIAL, LONG_TEMPLATE_ENTRY> {NUMBER_INTEGER}  { return NUMBER_INTEGER; }
<YYINITIAL, LONG_TEMPLATE_ENTRY> {NUMBER_UNSIGNED}  { return NUMBER_UNSIGNED; }
...
<APOS_STRING>                         \'                  { popState();                   return CLOSING_QUOTE; } // we keep this for chars
...
<QUO_STRING, APOS_STRING, THREE_QUO_STRING> {SHORT_TEMPLATE_ENTRY}      { pushState(SHORT_TEMPLATE_ENTRY);
                                                                                             yypushback(yylength() - 1);
                                                                                             return SHORT_TEMPLATE_ENTRY_START;}
// long string interpolation
<QUO_STRING, APOS_STRING, THREE_QUO_STRING> {LONG_TEMPLATE_ENTRY_START} { pushState(LONG_TEMPLATE_ENTRY);
